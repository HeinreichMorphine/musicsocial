        ...
        @forelse ($usersToSuggest as $suggestedUser)
            <div class="flex items-center justify-between mb-5 last:mb-0" x-data="{ followed: {{ auth()->user()->following->contains($suggestedUser) ? 'true' : 'false' }}, followersCount: {{ $suggestedUser->followers()->count() }} }" x-show="!followed" x-transition.duration.300ms>
                <div class="flex items-center min-w-0 flex-1 mr-3">
                    <x-user-avatar :user="$suggestedUser" class="w-12 h-12 mr-3 shrink-0 border border-white dark:border-gray-700 shadow-sm" />
                    <div class="min-w-0">
                ...
                ">
                    <button type="submit" 
                            ...
                            class="text-white text-xs font-bold py-2 px-5 rounded-full transition duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                    </button>
                </form>
            </div>
        @empty
