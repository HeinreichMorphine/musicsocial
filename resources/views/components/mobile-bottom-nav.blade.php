@auth
    @php
        ...
        $activeClasses = '!text-custom-mid-blue dark:!text-custom-mid-blue font-bold';
    @endphp
    ...
    <div class="md:hidden fixed bottom-0 left-0 z-50 w-full h-16 bg-white/90 dark:bg-black/90 backdrop-blur-lg border-t border-gray-200 dark:border-gray-800">
        <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
            ...
            <a href="{{ route('discovery') }}" wire:navigate class="{{ $baseClasses }} @if($isDiscovery) {{ $activeClasses }} @endif">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 mb-1 group-hover:scale-110 transition-transform @if($isDiscovery) text-custom-mid-blue @endif">
            ...
            <a href="{{ route('settings.index') }}" wire:navigate class="{{ $baseClasses }} @if($isSettings) {{ $activeClasses }} @endif">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 mb-1 group-hover:scale-110 transition-transform @if($isSettings) text-custom-mid-blue @endif">
                ...
                <span class="text-[10px] sm:text-xs">Settings</span>
            </a>
        </div>
    </div>
@endauth
