        ...
        <div class="absolute inset-0 z-30 flex items-center justify-center space-x-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <a href="{{ $song->spotify_url }}" 
               ...
               @if(auth()->check() && auth()->user()->spotify_token && auth()->user()->isSpotifyPremium())
                   @click.prevent="if(typeof window.toggleSpotifyPlayer !== 'undefined') { window.toggleSpotifyPlayer('spotify:track:{{ $song->spotify_track_id }}') } else { window.open('{{ $song->spotify_url }}', '_blank') }"
               @endif
