...
<div {{ $attributes->merge(['class' => 'bg-white rounded-2xl shadow-sm border border-gray-100 p-4 transition-all hover:shadow-md flex items-center justify-between w-full']) }}
    ...
    <div class="flex items-center min-w-0 flex-1 mr-2 overflow-hidden">
        <a href="{{ route('profile.show', $user->name) }}" class="shrink-0">
            <x-user-avatar :user="$user" class="h-10 w-10 border-2 border-white shadow-sm hover:opacity-90 transition-opacity" />
        </a>
        <div class="ml-2 min-w-0 flex-1">
            <a href="{{ route('profile.show', $user->name) }}" class="block text-sm font-bold text-gray-900 truncate hover:text-custom-mid-blue transition-colors">
                {{ $user->name }}
            </a>
            @if($user->username)
                <p class="text-xs text-gray-500 truncate">@<span>{{ $user->username }}</span></p>
            @endif
        </div>
    </div>
